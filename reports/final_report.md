...
黄玺主要负责实验 pipeline、环境配置、真实数据下载、预处理、聚类算法实现、指标计算和结果可视化。具体产出包括运行脚本、`outputs/metrics.csv`、`outputs/run_summary.md`、UMAP/t-SNE 图、指标图和 marker 表格。

叶浩林主要负责 PBMC 背景、marker gene 解释、细胞类型注释、图注撰写、讨论部分和报告整合。最终报告中第 1 节背景、第 5 节细胞群解释、第 6 节讨论和图注部分主要对应叶浩林的工作。
